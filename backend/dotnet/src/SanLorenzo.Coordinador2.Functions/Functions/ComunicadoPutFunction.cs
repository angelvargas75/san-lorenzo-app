...
public sealed class ComunicadoPutFunction
{
    ...
    {
        if (!ObtenerComunicadoId(
                request,
                out var comunicadoId))
        {
            return ApiResponseFactory.Create<object?>(
                400,
                "El identificador del comunicado no es válido.",
                null);
        }
        ...
        try
        {
            ...
            try
            {
                ...
                if (comunicado is null)
                {
                    ...
                    return ApiResponseFactory.Create<object?>(
                        404,
                        "No se encontró el comunicado.",
                        null);
                }

                await EliminarDestinatariosAsync(
                    connection,
                    transaction,
                    comunicadoId);
        ...
        {
            context.Logger.LogLine(
                $"Error al actualizar el comunicado: {exception}");
    ...
    private static async Task EliminarDestinatariosAsync(
        SqlConnection connection,
        SqlTransaction transaction,
        int comunicadoId)
    {
        const string sql = """
            DELETE FROM dbo.ComunicadoDestinatario
            WHERE ComunicadoId = @ComunicadoId;
            """;
    ...
    private static bool ObtenerComunicadoId(
        APIGatewayHttpApiV2ProxyRequest request,
        out int comunicadoId)
    {
        comunicadoId = 0;
